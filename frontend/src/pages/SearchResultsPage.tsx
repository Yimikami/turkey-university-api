...
import { ArrowLeftIcon, AcademicCapIcon, BuildingLibraryIcon } from '@heroicons/react/24/outline';
...
const SearchResultsPage = () => {
  ...
  const [searchType, setSearchType] = useState<'faculty' | 'program'>(initialSearchType);
    ...
    if (facultyResults.length === 0 && !loading && searchQuery) {
      return (
        <div className="bg-yellow-100 border border-yellow-400 text-yellow-700 px-4 py-3 rounded relative" role="alert">
          <span className="block sm:inline">Arama kriterlerinize uygun fakülte bulunamadı.</span>
        </div>
      );
    }

    return facultyResults.map((result) => (
      <div key={result.id} className="bg-white rounded-lg shadow-md p-6 mb-4">
  ...
  const renderProgramResults = () => {
    if (programResults.length === 0 && !loading && searchQuery) {
      return (
        <div className="bg-yellow-100 border border-yellow-400 text-yellow-700 px-4 py-3 rounded relative" role="alert">
          <span className="block sm:inline">Arama kriterlerinize uygun program bulunamadı.</span>
        </div>
      );
    }

    return programResults.map((result) => (
      <div key={result.id} className="bg-white rounded-lg shadow-md p-6 mb-4">
        <div className="flex items-center justify-between mb-4">
          <h3 className="text-xl font-semibold">{result.name}</h3>
          <span className={`px-3 py-1 rounded-full text-xs font-medium ${
    ...
    <div className="max-w-5xl mx-auto">
      ...
      <div className="mb-8">
        ...
        <SearchBar 
          ...
          placeholder={searchType === 'faculty' ? 'Fakülte ara...' : 'Program ara...'}
          className="max-w-3xl"
        />
      </div>
      ...
      {loading ? (
        ...
        <div>
          {searchType === 'faculty' ? renderFacultyResults() : renderProgramResults()}
        </div>
      )}
    </div>
  );
